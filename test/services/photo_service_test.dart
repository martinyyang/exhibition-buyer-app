  ...
  group('PhotoService - 照片上传与存储', () {
    ...
    group('uploadPhoto - 照片上传', () {
      test('成功上传照片并创建记录（不含供应商信息）', () async {
        ...
        when(() => mockSupabase.from('photos'))
            .thenReturn(mockQuery as dynamic);
        ...
        verify(() => mockQuery.insert(
            any(that: containsPair('booth_id', testBoothId)))).called(1);
      });
      ...
      test('成功上传照片并创建记录（含供应商信息）', () async {
        ...
        when(() => mockBucket.upload(any(), any(),
                fileOptions: any(named: 'fileOptions')))
            .thenAnswer((_) async => 'photos/test-file.jpg');
        when(() => mockBucket.getPublicUrl(any()))
            .thenReturn('https://storage.supabase.co/photos/test-file.jpg');
    ...
    group('updatePhoto - 更新照片信息', () {
      test('更新供应商名称', () async {
        ...
        verify(() => mockQuery
            .update(containsPair('supplier_name', newSupplierName))).called(1);
      });
      ...
      test('更新供应商Logo URL', () async {
        ...
        verify(() =>
                mockQuery.update(containsPair('supplier_logo_url', newLogoUrl)))
            .called(1);
      });
    ...
    group('deletePhoto - 删除照片', () {
      test('成功删除照片（同时删除Storage文件和数据库记录）', () async {
        final mockQuery = MockPostgrestFilterBuilder();
        const photoUrl =
            'https://storage.supabase.co/object/public/photos/team-789/booth-123/1234567890_uuid.jpg';
        ...
        verify(() =>
                mockBucket.remove(any(that: contains('team-789/booth-123/'))))
            .called(1);
    ...
    group('uploadSupplierLogo - 上传供应商Logo', () {
      test('成功上传供应商Logo并返回URL', () async {
        final mockFile = MockFile();
        const expectedUrl =
            'https://storage.supabase.co/suppliers/1234567890_uuid.jpg';
